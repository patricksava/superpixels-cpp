...
Pixel Image::rgbToLab( Pixel rgb )
{
    return XYZToLab( rgbToXYZ( rgb ) );
}

Pixel Image::LabTorgb( Pixel Lab )
{
...
void Image::convertImageFromRGB2Lab( void )
{
    for (int i = 0; i < _w * _h; i++) {
        Pixel rgbPx = _data[i];
        Pixel xyzPx = rgbToXYZ(rgbPx);
        Pixel labPx = XYZToLab(xyzPx);
...
void Image::convertImageFromLab2RGB( void )
{
    for (int i = 0; i < _w * _h; i++) {
        Pixel labPx = _data[i];
        Pixel xyzPx = LabToXYZ(labPx);
        Pixel rgbPx = XYZTorgb(xyzPx);
